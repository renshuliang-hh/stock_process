...
def match_index(name, year, data):
    ...
    for index, row in data_year.iterrows():
        if row['city'] in name or name in row['city']:
            try:
                y1_pr, stock = row['y1_pr'], row['register']
            except Exception:
                y1_pr, stock = np.nan, np.nan
...
def match_index_2(name, year, data,POP):
    ...
    for index, row in data_year.iterrows():
        if row['city'] in name or name in row['city']:
            try:
                y2_pr, stock = row['y2_pr'], row['register']
            except Exception:
                y2_pr, stock = np.nan, np.nan
...
def combin_data(data):
    print("拼接第一部分数据")
    relation = pd.read_csv('re_method\省市对应表.csv', engine='python')
    ...
    for year in range(2002, 2021):
        ...
        add_data2_pd = pd.DataFrame({str(year) + 'register': add_data_list2})
        relation[str(year) + 'y1_pr'] = add_data1_pd
        relation[str(year) + 'register'] = add_data2_pd
...
def combin_data_2(data):
    ...
    data_city_pop = pd.read_csv('city_data_spatiotemporal_withbaidu_light.csv',encoding='GBK')
